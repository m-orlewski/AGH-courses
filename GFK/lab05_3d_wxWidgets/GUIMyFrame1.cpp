...
void GUIMyFrame1::Repaint()
{
    ...
    Matrix4 scale = scaleMatrix(static_cast<double>(w / 2) * WxSB_ScaleX->GetValue() / 200.0, static_cast<double>(h / 2) * WxSB_ScaleY->GetValue() / 200.0, 5.0 * WxSB_ScaleZ->GetValue());
    ...
    Matrix4 translation = translationMatrix((WxSB_TranslationX->GetValue() - 100.0) / 200.0 * w, (WxSB_TranslationY->GetValue() - 100.0) / 200.0 * h, (WxSB_TranslationZ->GetValue() - 100.0) * 10);
    ...
    for (auto& x : data)
    {
        ...
        buffered_dc.DrawLine(start.GetX() + w / 2, start.GetY() + h / 2, end.GetX() + w / 2, end.GetY() + h / 2);
    }
}
